...
class Main
{
    ...
    void go() throws Exception
    {
        ...
        {
            int value = guardSleepCount.get(guard);
            if(maxSleep < value)
            {
                maxSleep = value;
        ...
        System.out.println("Sleepiest guard is " + sleepiestGuard + " and they slept " + maxSleep + " minutes.");

        int[] minuteCount2 = guardMinuteCount.get(sleepiestGuard);
        int maxMinute = -1;
        int maxMinuteSleep = -1;
        for(int k=0; k<59; k++)
        {
            if(minuteCount2[k] > maxMinuteSleep)
            {
                maxMinute = k;
                maxMinuteSleep = minuteCount2[k];
            }
        }
        System.out.println("Sleepiest minute is " + maxMinute + " where they slept " + maxMinuteSleep + " times");
        System.out.println("Answer is: " + (sleepiestGuard * maxMinute));
        // for(int guard : guardSleepCount.keySet())
        // {
