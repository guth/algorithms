...
class Main
{
    ...
    void go() throws Exception
    {
        ...
        while(keepGoing)
        {
            ...
            for(int i=0; i<carts.length; i++)
            {
                ...
                for(int j=0; j<carts.length; j++)
                {
                    ...
                    if(carts[i].x == carts[j].x && carts[i].y == carts[j].y)
                    {
                        ...
                        System.out.println("Carts left: " + carts.length);
            ...
            // System.out.println(numNotCollided);
            ...
            Cart[] newCarts = new Cart[numNotCollided];
            int place = 0;
            for(Cart cart : carts)
            {
                if(!cart.collided)
    ...
    private void printUpdate(char[][] input, Cart[] carts)
    {
        for(int y=0; y<maxY; y++)
        {
            for(int x=0; x<maxX; x++)
            {
                ...
                for(Cart cart : carts)
                {
                    if(!cart.collided && cart.x == x && cart.y == y)
                    {
                        if(cart.dir == RIGHT)
                            c = '>';
                        if(cart.dir == LEFT)
